...
- **ReactFire** — React bindings for Firebase; previously used here for live RTDB reads, since removed (unmaintained, CJS `require("react")` crash under Vite's rolldown bundler) in favor of `firebase/database`'s `onValue` directly. See [[ADR-0006-frontend-live-sync]].
